...
| D-1006 | `Discarded` |
| D-1007 | Primo giro: ⚠️ `ApprovalPending` con `InsufficientStock` (vedi sotto). Dopo D61, su database ricreato: `Failed` in 36 s con la nota `Host: SKU inesistenti in ERP: IND-SEN-999. Ordine non proponibile, nessuna approvazione (D20).`, nessuna richiesta in `/approvals`, nessun ordine. Anche in questo giro il modello non ha segnalato lo SKU: l'arresto l'ha deciso l'host |
