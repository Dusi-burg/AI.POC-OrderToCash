...
| D52 | **Manopole della demo inoltrate dall'AppHost**: `MODEL_PROVIDER`, `ANTHROPIC_MODEL`, `OLLAMA_MODEL`, `OLLAMA_ENDPOINT`, `O2C_AGENT_MODE`, `APPROVAL_THRESHOLD_EUR`, `APPROVAL_TIMEOUT_HOURS`, `APPROVAL_SWEEP_MINUTES` e `O2C_PROMPT_CAPTURE_DIR` (aggiunta il 2026-09-18 con D62) all'orchestratore, `Approvals__ApproverUpn` e `Approvals__CrmDevBaseUrl` ad Approvals.Web, prese dalla configurazione dell'AppHost: una variabile si imposta in un punto solo (2026-09-16) | 5 |
...
| D60 | **Correzioni emerse dai test della Fase 6** (2026-09-17): (a) in `Crm.Web` i retry del resilience handler standard sono disattivati per i metodi non sicuri con `ConfigureAll`, perché le opzioni registrate da `ConfigureHttpClientDefaults` non portano il nome del client e un retry dopo un 503 diventava un 409; (b) `ToolProblems.ConfigureExceptionHandler` fa rispondere 400 a un corpo JSON illeggibile, che in Development diventava 500, su `Crm.Mcp` ed `Erp.Api`; (c) `Erp.Web` rifiuta con 405 ogni metodo diverso da GET/HEAD. Emerso dal vivo su D-1007 e corretto con D61 | 6 |
| D61 | **SKU inesistente fermato dall'host** (utente, 2026-09-17, dopo il run dal vivo di D-1007): se la verifica di giacenza di `ApprovalGate` risponde `NOT_FOUND`, il gate restituisce un verdetto di arresto `Failed` dell'host invece di contare la riga come non disponibile; il motore lo registra, rifiuta la chiamata a `create_order` e l'esito lo scrive `CompleteAsync`. La guardia dei tool, dopo un verdetto di arresto, respinge con `CONFLICT` le scritture degli agenti (`create_customer`, `create_order`, `update_deal`), non quelle dell'host: altrimenti un `update_deal OrderCreated` del modello dopo il rifiuto lascerebbe sul CRM uno stato falso (M28) | 5, 6 |
| D62 | **Gli agenti si scambiano fatti, non testo** (utente, 2026-09-17, dall'analisi dei run falliti su D-1007 e D-1003): nel workflow di handoff il testo con cui un agente accompagna il passaggio di mano arriva al successivo come messaggio dell'utente. Con `qwen3.5:9b` alcune formulazioni di `IntakeAgent` (“…so I can hand off to FulfillmentAgent”, “…immediately”) facevano passare la mano a `OrderAgent` senza che `FulfillmentAgent` avesse chiamato `check_stock`: il guardrail teneva (le giacenze le riverifica l'host, D54) ma un agente non faceva il proprio lavoro. Le istruzioni da sole non bastano, perché quel testo lo produce un altro modello e cambia a ogni run. `ForeignAgentTextFilter` toglie dalla richiesta al modello il **testo** dei messaggi scritti dagli altri agenti del workflow e lascia le loro chiamate ai tool e i risultati, che sono i fatti su cui l'agente lavora; la conversazione del workflow e i checkpoint restano completi. Misure: D-1007 2/10 → 10/10, D-1003 4/10 → 9/10 con le sole istruzioni e 10/10 con il filtro; replay 20/20 deterministico sulla conversazione incriminata. Con il filtro attivo le istruzioni difensive degli agenti sono state tolte (2026-09-18): replay 30/30 su sei conversazioni, che senza filtro falliscono in quattro casi su sei, 0/5 ciascuno e sempre con l'handoff al posto di `check_stock`. Dal vivo su qwen, **un giro per deal** su D-1001, D-1002, D-1003 e D-1006 (copertura accettata dall'utente, il replay copre la regressione): tutti corretti, `check_stock` chiamato su ogni riga prima di ogni handoff e righe di `create_order` identiche a quelle del deal | 4, 6 |
| D63 | **Le specifiche degli agenti in file leggibili** (utente, 2026-09-18): il prompt di ogni agente esce dal codice e diventa un documento Markdown per agente in `src/Dusiburg.AI.O2C.Orchestrator/Agents/Specs`, incluso come **risorsa dell'assembly** (nessun file letto dal disco a runtime). Il titolo dà il nome, la citazione sotto il titolo la descrizione, e ogni `## Sezione` è un testo che il modello riceve; `## Note (non inviate al modello)` è commento per chi legge e non arriva da nessuna parte, così le spiegazioni in italiano convivono con un prompt che resta in inglese (G3.3). Nel codice restano l'allow-list dei tool, il verdetto di arresto e la topologia: sono guardrail, e un refuso lì deve restare un errore di compilazione invece di diventare in silenzio un permesso allargato per un agente. Siccome il compilatore non sorveglia più il testo, lo fanno i test (`AgentSpecTests`): una specifica incompleta o malformata fa fallire la build. Vale anche per l'agente singolo della Fase 3. Limite accettato: essendo risorse, cambiare una parola richiede comunque di ricompilare | 3, 4, 6 |
| D64 | **Le regole di approvazione si leggono senza saper programmare** (utente, 2026-09-18): `ApprovalPolicy` passa da quattro `if` a un elenco dichiarativo di `ApprovalRule`, ognuna con nome e spiegazione in italiano, e da quell'elenco un test genera `docs/regole-di-approvazione.md`. Il documento non può divergere da ciò che il sistema fa, perché nasce dalla stessa lista che decide: se qualcuno cambia una regola senza rigenerarlo, il test lo riscrive e fallisce, e la differenza si vede in git. Le regole **restano compilate**, a differenza dei prompt di D63: questo è il punto che decide se servono una firma e dei soldi, quindi cambiarne una deve costare una modifica al codice, una build e dei test, non la riga di un file di dati; configurabile resta la sola soglia (`APPROVAL_THRESHOLD_EUR`), come già oggi. Valutato e scartato il Gherkin eseguibile con Reqnroll: l'artefatto sarebbe più vistoso in demo, ma il pacchetto è compilato su NUnit 3 mentre la solution usa NUnit 4 su Microsoft.Testing.Platform, quindi prima servirebbe uno spike | 5, 6 |
...
| M28 | §7 | SKU inesistente scoperto dalla verifica dell'host → `Failed` senza approvazione; nessuna scrittura degli agenti dopo un verdetto di arresto | D61 | ✅ Applicata in F6 (`docs/architettura.md`) |
| M29 | §5 | Gli agenti si scambiano fatti, non testo: il testo di un agente non entra nella richiesta al modello del successivo; restano le chiamate ai tool e i risultati (`ForeignAgentTextFilter`) | D62 | ✅ Applicata in F6 (`docs/architettura.md`) |
| M30 | §5, §10 | Le istruzioni degli agenti stanno in `Agents/Specs/<Nome>.agent.md`, incluse come risorse dell'assembly; nel codice restano allow-list dei tool, verdetti di arresto e topologia | D63 | ✅ Applicata (2026-09-18) |
| M31 | §7 | Le regole di approvazione sono un elenco dichiarativo con nome e spiegazione; da esse si genera `docs/regole-di-approvazione.md`, tenuto allineato da un test. Le regole restano compilate, configurabile la sola soglia | D64 | ✅ Applicata (2026-09-18) |
